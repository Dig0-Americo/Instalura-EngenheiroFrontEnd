        ...
        return(
            <div className="main-container">
                ...
                <form onSubmit={(e) => {e.preventDefault(); this.props.submitSignup(e.target, this.login.value, this.senha.value, this.confirmPassword.value, this.profileUrl.value)}}>
                    ...
                    <input placeholder="Senha" required type="password" autoComplete onKeyUp={() => this.props.validatePassword(this.login.value, this.senha.value, this.confirmPassword.value)} ref={(input) => this.senha = input}/>
                    <input placeholder="Confirmar senha" required type="password" autoComplete onKeyUp={() => this.props.validatePassword(this.login.value, this.senha.value, this.confirmPassword.value)} ref={(input) => this.confirmPassword = input}/>
                    <input placeholder="Url do perfil: https://github.com/nomeUsuario" required type="url" onKeyUp={() => this.props.validateUrl(this.profileUrl.value)} ref={(input) => this.profileUrl = input}/>
